...
func (m *Message) Marshal() ([]byte, error) {
	...
	if bodyBytes, err = tlv.Marshal(m.Body); err != nil {
		return nil, fmt.Errorf("n41: marshal message type %d body failed: %s", m.Header.MessageType, err)
	}
	...
	if headerBytes, err = m.Header.MarshalBinary(); err != nil {
		return nil, fmt.Errorf("n41: marshal message type %d header failed: %s", m.Header.MessageType, err)
	}
